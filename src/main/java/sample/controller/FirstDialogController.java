...
public class FirstDialogController implements Initializable {
    ...
    @FXML
    public void handleGetMainDescriptor(ActionEvent event) throws IOException, JAXBException {
        Stage stage = new Stage();
        stage.initModality(Modality.WINDOW_MODAL);

        FileChooser fileChooser = new FileChooser();

        File mainTableauDescriptor = fileChooser.showOpenDialog(stage);
        if (isDescriptorValid(mainTableauDescriptor)) {
            // TODO: DEV HANDLING HERE

            List<DataContainerDefinitionFile> tableauList = loadTableausFromDescriptor(mainTableauDescriptor);
            Controller.setTableauChooserList(tableauList);

            InstanceLoader instanceLoader = new InstanceLoader();
            instanceLoader.loadInstances(mainTableauDescriptor.getParentFile());
            Controller.setInstanceLoader(instanceLoader);
...
//            pb.start();
    ...
    private List<DataContainerDefinitionFile> loadTableausFromDescriptor(File descriptorFile) throws JAXBException {
        JAXBContext context = JAXBContext.newInstance(DataContainerDescriptor.class);
        Unmarshaller um = context.createUnmarshaller();
        DataContainerDescriptor descriptor = (DataContainerDescriptor)um.unmarshal(descriptorFile);

        return descriptor.getFiles();
    }
